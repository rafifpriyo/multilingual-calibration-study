...
import lm_eval
import shutil
if not os.path.exists(f"{os.path.join(os.path.dirname(lm_eval.__file__), f'tasks/global_piqa/')}"):
    shutil.copytree(f"{os.path.dirname(__file__)}/global_piqa", f"{os.path.join(os.path.dirname(lm_eval.__file__), f'tasks/global_piqa/')}")


import lm_eval
update_yaml_path = f"{os.path.join(os.path.dirname(lm_eval.__file__), f'tasks/global_piqa/completions/_template')}"
update_yaml_file = '''
dataset_path: mrlbenchmarks/global-piqa-nonparallel-v0.1
...
def eval_model(model, device='cpu'):
  return simple_evaluate(
      model=model,
      tasks=[f"global_piqa_completions_{lang}" for lang in eval_languages],
            # "xwinograd",
            #  "xstorycloze"],
      device=device,
