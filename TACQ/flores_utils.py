...
import torch
from gptq.datautils import get_flores
class Flores_Dataset(torchDataset):
    def __init__(
        self, seed, use_train_split, tokenizer_name, subset=None, verbose=False, \
        nsamples=128, seqlen=2048, device="cuda"
    ):
        self.seqlen = seqlen
        self.use_train_split = use_train_split
        train_loader, testenc = get_flores(nsamples=nsamples, seed=seed, seqlen=seqlen, model=tokenizer_name, subset=subset)
        if use_train_split:
