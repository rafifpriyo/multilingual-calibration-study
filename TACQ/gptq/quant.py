...
def quantize(x, scale, zero, maxq):  # Normal Linear Quantization
    if maxq < 0:
...
class Quantizer(nn.Module):
    ...
    def find_params(self, x, weight=False):
        ...
        shape = x.shape
        if self.perchannel:
            if weight:  # For weight only quantization, there is no special reshaping.
                x = x.flatten(1)
            else:
                if len(shape) == 4:
                    x = x.permute([1, 0, 2, 3])
        ...
        # print(f"DEBUG: self.scale.shape {self.scale.shape} rows")
        if self.mse:
            best = torch.full([x.shape[0]], float('inf'), device=dev)
            for i in range(int(self.maxshrink * self.grid)):  # Grid is default 100, maxshrink is default .8
                p = 1 - i / self.grid 
                xmin1 = p * xmin
                xmax1 = p * xmax
        ...
        if weight:
            shape = [-1] + [1] * (len(shape) - 1)  # [-1, 1] len(shape) is the number of dimensions in weight tensor. 
            self.scale = self.scale.reshape(shape)  # Vector of [channels, 1]
            # print(f"VERIFY: self.scale.shape {self.scale.shape} == channels or rows x 1")
