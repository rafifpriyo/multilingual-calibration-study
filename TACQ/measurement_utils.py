...
from datasets_directory.PretrainDatasets.flores_utils import Flores_Dataset
...
def preprocess_calibration_datasets(args, tokenizer, indices_for_choices, n_calibration_points=128, seqlen=2048):
    ...
    if args.dataset == "MMLU_MCQA":
    ...
    elif args.dataset.startswith("flores"):
        ...
        if subset in lang_mapper:
            train_dataset = Flores_Dataset(seed=args.serial_number, use_train_split=True, tokenizer_name=tokenizer.name_or_path, subset=lang_mapper[subset], verbose=False, nsamples=n_calibration_points, seqlen=seqlen, device="auto")
        else:
            raise ValueError(f"Dataset Flores {subset} not considered supported yet")
    else:
