...
for lang in eval_languages:
    import lm_eval
    update_yaml_path = f"{os.path.join(os.path.dirname(lm_eval.__file__), f'tasks/xnli/xnli_{lang}')}"
    ...
    for file in [f for f in os.listdir(os.path.dirname(update_yaml_path))]:
        ...
        if 'description' not in data:
            data['description'] = 'You are a language expert and you always know if two sentences entail, neutral, or contradict one after the other.\n\
You are tasked to generate two sentences and calculate how probable the sentences to be an entailment, neutral, or contradiction statement\n\
Write the conclusion in the middle of the two sentences correspondingly to each language used\n\
Learn from the pattern and choose correctly'
...
def eval_model(model, device='cpu'):
  return simple_evaluate(
      model=model,
      tasks=[f"xnli_{lang.lower()}" for lang in eval_languages],
            # "xwinograd",
            #  "xstorycloze"],
      device=device,
