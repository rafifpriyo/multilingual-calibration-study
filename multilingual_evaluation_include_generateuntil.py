...
"""
    ...
    for file in [f for f in os.listdir(os.path.dirname(update_yaml_path))]:
        ...
        if 'aggregate_metric_list' in data:
            data['aggregate_metric_list'][0] = {'metric': 'exact_match', 'weight_by_size': True, 'aggregation': 'mean', 'filter_list': 'custom-extract'}
            with open(file_name, 'w') as f:
                yaml.dump(data, f)

    for file in [f for f in os.listdir(os.path.dirname(update_yaml_path_2))]:
        if not file.endswith('.yaml'):
            continue
            
        file_name = os.path.join(os.path.dirname(update_yaml_path_2), file)
        with open(file_name, 'r') as f:
            data = yaml.load(f)
        if 'description' in data:
            data['description'] = 'Answer directly with the choice: A, B, C or D without explanation as shown in the example\n'
...
wandb_runname = f"{model_id.split('/')[-1]}-{quantization_technique}-{bit}bit-{lang}-{evaluation_dataset}-{'think' if enable_thinking else 'nothink'}-generateuntil"
